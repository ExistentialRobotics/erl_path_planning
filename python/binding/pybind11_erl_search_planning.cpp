...
BindAMRAStar(py::module &m) {
    ...
    py::class_<amra_star::Output, std::shared_ptr<amra_star::Output>>(amra_star_module, "Output")
        .def_readwrite("goal_index", &amra_star::Output::goal_index)
        .def_readwrite("path", &amra_star::Output::path)
        .def_readwrite("action_coords", &amra_star::Output::action_coords)
        .def_readwrite("cost", &amra_star::Output::cost)
