...
struct AppData {
    std::string window_name = "DubinsPath";
...
OnMouse(int event, int x, int y, int flags, void *userdata) {
    ...
    ERL_INFO("dubins path: %s, length: %f", app_data->reeds_shepp_path->GetDubinsPathType().c_str(), app_data->reeds_shepp_path->GetLength());
