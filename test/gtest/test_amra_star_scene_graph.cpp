...
TEST(ERL_SEARCH_PLANNING, AMRAStarSceneGraph_SingleFloor) {
    ...
    std::vector<std::pair<std::shared_ptr<erl::search_planning::HeuristicBase>, std::size_t>> heuristics = {
        {std::make_shared<erl::search_planning::EuclideanDistanceHeuristic<3>>(goal, goal_tolerance), 0},  // anchor
        {std::make_shared<erl::search_planning::EuclideanDistanceHeuristic<3>>(goal, goal_tolerance), 1},  // kNA
        {std::make_shared<erl::search_planning::EuclideanDistanceHeuristic<3>>(goal, goal_tolerance), 2},  // kObject
        {std::make_shared<erl::search_planning::EuclideanDistanceHeuristic<3>>(goal, goal_tolerance), 3},  // kRoom
        {std::make_shared<erl::search_planning::EuclideanDistanceHeuristic<3>>(goal, goal_tolerance), 4},  // kFloor
    };
...
TEST(ERL_SEARCH_PLANNING, AMRAStarSceneGraph_CrossFloor) {
    ...
    double path_cost = result->costs[result->latest_plan_itr];
    std::cout << "Path cost: " << path_cost << std::endl;
    EXPECT_NEAR(path_cost, 15.371834, 1e-6);
    if (amra_setting->log) { result->Save(output_dir / "amra.solution"); }
...
TEST(ERL_SEARCH_PLANNING, AMRAStarSceneGraph_LinearTemporalLogic) {
    ...
    auto ltl_heuristic = std::make_shared<erl::search_planning::LinearTemporalLogicHeuristic3D>(
    ...
    double path_cost = result->costs[result->latest_plan_itr];
    std::cout << "Path cost: " << path_cost << std::endl;
    EXPECT_NEAR(path_cost, 18.482966, 1e-6);
    if (amra_setting->log) { result->Save(output_dir / "amra.solution"); }
